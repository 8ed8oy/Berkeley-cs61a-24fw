...
def boar_brawl(player_score, opponent_score):
    ...
    player_score:     The total score of the current player. Use the ones digit.
    opponent_score:   The total score of the other player. Use the tens digit.

    """
    # BEGIN PROBLEM 2
    ones_digit = player_score % 10
    tens_digit = opponent_score // 10 % 10
    score = 3 * abs(ones_digit - tens_digit)
    if score == 0:
        return 1
    else:
        return score
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            roll_time = strategy0(score0, score1)
            score0 = update(roll_time, score0, score1, dice)
            who = 1
        else:
            roll_time = strategy1(score1, score0)
            score1 = update(roll_time, score1, score0, dice)
            who = 0
    # END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    best_num_rolls = 1
    for i in range(10):
        average = make_averaged(roll_dice, times_called)(i + 1, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if sus_points(new_score) - score >= threshold:
        num_rolls = 0
    return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    roll = 6
    if boar_strategy(score, opponent_score, 9, roll) == 0:
        return 0
    make_averaged(roll_dice, times_called=100)
    for i in range(1, 6):
        if make_averaged(roll_dice, times_called=100)(i, six_sided) > GOAL - score:
            roll = i
            break
    return roll
    # END PROBLEM 12
