...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive:
            if cur_place.bees and distance_count >= self.lower_bound and distance_count <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0:
            for bee in place.bees[:]:  # Iterate over a copy of the list
...
# BEGIN Problem 6
class WallAnt(Ant):
    '''A WallAnt is useless because it has alow health value originally'''
...
# BEGIN Problem 7
class HungryAnt(Ant):
    '''Fake Chomper ,but reinforced, i think ,which is still useless?'''
    ...
    def action(self, gamestate):
        if self.place.bees and self.cooldown == 0:
            self.bite(random_bee(self.place.bees))
...
class TankAnt(ContainerAnt):
    """Provide protection and offer damage at the same time"""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while cur_place.exit:
            ...
            if cur_place.ant and cur_place.ant.is_doubled == False:
                cur_place.ant.double()
                if cur_place.ant.is_container and cur_place.ant.ant_contained and cur_place.ant.ant_contained.is_doubled == False:
                    cur_place.ant.ant_contained.double()
            elif cur_place.ant and cur_place.ant.is_container and cur_place.ant.ant_contained and cur_place.ant.ant_contained.is_doubled == False:
                cur_place.ant.ant_contained.double()
        # END Problem 12
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 12
        super(QueenAnt, self).reduce_health(amount)
        if self.health <= 0:
            ants_lose()
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not target:
            return
        target.slow_turns = 5
        def slowed_action(gamestate, bee=target):
            if bee.slow_turns > 0:
                ...
                target.action = bee.original_action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while current is not None and not current.is_hive:
            ant = current.ant
            visited = set()
            while ant is not None and ant not in visited:
                visited.add(ant)
                if ant is not self:
                    insects.setdefault(ant, distance)
                if ant.is_container == False and getattr(ant, 'ant_contained', None) is not None:
                    ant = ant.ant_contained
                else:
                    break
            for bee in current.bees:
                insects.setdefault(bee, distance)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if ant.is_container and ant.ant_contained is not None:
            return ant.ant_contained.blocks_path
        return False
        # END Problem EC 3
    ...
    def action(self, gamestate):
        ...
        if self.is_scared:
            ...
            self.scare_actions_remaining -= 1
            if self.scare_actions_remaining == 0:
    ...
    def original_action(self, gamestate):
        """A Bee's action stings the Ant that blocks its exit if it is blocked,
        or moves to the exit of its current place otherwise.
    ...
    def scare(self, length):
        ...
        if not self.has_been_scared:
            ...
            self.is_scared = True
            self.scare_actions_remaining = length
        # END Problem EC 2
