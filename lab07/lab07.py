...
class Account:
    ...
    def time_to_retire(self, amount: float) -> int:
        ...
        while future_balance < amount:
            future_balance = future_balance * (1 + self.interest)
            years += 1
        return years
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount: int) -> int | str:
        current_withdraw_fee = self.withdraw_fee if self.withdraw_times > 1 else 0
        ...
        if amount > self.max_withdrawal:
            self.withdraw_times+=1
            return "Can't withdraw that amount"
        self.balance = self.balance - amount - current_withdraw_fee
        self.withdraw_times+=1
        return self.balance
