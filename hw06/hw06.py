    ...
    def vend(self) -> str:
        ...
        if self.stock == 0:
            ...
            if self.balance < self.price:
                ...
                self.balance = 0
                self.stock -= 1
                return f'Here is your {self.product_name} and ${balance_before - self.price} change.'
...
def store_digits(n: int):
    ...
    def helper(x, d):
        ...
        first = x // p
        rest = x % p
        return Link(first, helper(rest, d - 1))

    return helper(n, len_num(n))
...
def two_list(vals, counts):
    ...
    if counts[0] != 1:
        ...
        counts = counts[1:]
    if vals:
        return Link(label, two_list(vals, counts))
    else:
        return Link(label)
