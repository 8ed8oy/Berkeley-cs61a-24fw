...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
      ...
      pruned_leaves = [x for x in pruned_leaves if x]
      if not pruned_leaves:
        return tree(label(t))
      else:
...
def sum_tree(t):
    ...
    if is_leaf(t):
      ...
      for i in branches(t):
        sum += sum_tree(i)
      return sum
    #return label(t) if is_leaf(t) else sum(sum_tree(x) for x in branches(t)) + label(t)
...
def balanced(t):
    ...
    if is_leaf(t):
      ...
      for i in branches(t)[1:]:
        if balanced(i):
          current = sum_tree(i)
          if current != first_sum:
            return False
          #all_same = all(s == first_sum for s in branch_sums[1:])
        else: 
          return False
      return True  
    # return True if is_leaf(t) else all(balanced(b) for b in branches(t)) and len({sum_tree(b) for b in branches(t)}) <= 1 
    # using set to check if all the sum are the same is awesome but inefficient
