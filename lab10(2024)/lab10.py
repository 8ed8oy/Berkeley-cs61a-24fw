...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
       ...
       return bindings[exp] if isinstance(bindings[exp], (int, float, bool)) else calc_apply(bindings[exp], operands) # UPDATE THIS FOR Q4, how do you access a variable?
...
def eval_and(expressions):
    ...
    while next != nil:
        number = calc_eval(next.first)
        if number is scheme_f:
            return scheme_f
        ...
        next = next.rest
    return max_number if isinstance(number, (int, float)) else scheme_t
