...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    if len(typed) == 1 or len(source) == 1:
        if typed[0] == source[0] and len(typed) + len(source) - 2 < limit + 1:
            return len(typed) + len(source) - 2
        elif typed[0] == source[0] and len(typed) + len(source) - 2 > limit:
            return limit + 1
        elif len(typed) + len(source) - 1 < limit + 1:
            return len(typed) + len(source) - 1
        else:
            return limit + 1
    back = furry_fixes(typed[1:], source[1:], limit)
    if source[0] != typed[0] and back <= limit:
        return furry_fixes(typed[1:], source[1:], limit - 1) + 1
    elif source[0] != typed[0] and back > limit:
        return limit + 1
    else:
        return back
    # END PROBLEM 6
